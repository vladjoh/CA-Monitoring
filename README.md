...
Detects when a new Conditional Access policy is created. Adds a `RiskFlag` that automatically highlights if the policy was created by an app or service principal (no UPN) rather than a human — that's a red flag worth catching immediately.
...
Detects when a Conditional Access policy is permanently deleted. Adds a `RiskFlag` that automatically highlights if the deletion was performed by an app or service principal (no UPN) rather than a human — that's a red flag worth catching immediately.
